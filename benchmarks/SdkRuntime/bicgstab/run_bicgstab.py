...
""" test BiCGSTAB of a sparse matrix A built by 7-point stencil
  ...
  ---
       ...
       toc()       // record end time
  ---
  The run_bicgstab.py performs CG on the WSE, not calls a sequence of spmv and dot.
...
# How to compile
#   python run_bicgstab.py -m=5 -n=5 -k=5 --latestlink latest --channels=1 \
...
def main():
  ...
  simulator.launch("f_bicgstab", np.int16(zDim), np.float32(tol), np.int16(max_ite), nonblock=False)
  ...
  print(f"==== measure the residual b - A*x by |.|_inf")
  # r = b - A*xf
