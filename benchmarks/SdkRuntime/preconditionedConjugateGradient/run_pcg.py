...
""" test Preconditioned Conjugate Gradient of a sparse matrix A built by 7-point stencil
  ...
  ---
       ...
       toc()       // record end time
  ---
  The run_pcg.py performs PCG on the WSE, not calls a sequence of spmv and dot.
...
# How to compile
#   python run_pcg.py -m=5 -n=5 -k=5 --latestlink latest --channels=1 \
...
def main():
  ...
  simulator.launch("f_pcg", np.int16(zDim), np.float32(tol), np.int16(max_ite), nonblock=False)
