...
""" test power method of a sparse matrix A built by 7-point stencil
  ...
  ---
       ...
       toc()       // record end time
  ---
  The run_power.py performs power mthod on the WSE, not calls a sequence of spmv and nrm2.
...
# Note that debug_util can only be used in simulator runs
from cerebras.sdk.debug.debug_util import debug_util # pylint: disable=no-name-in-module
...
# How to compile
#   python run_power.py -m=5 -n=5 -k=5 --latestlink latest --channels=1 \
...
def main():
  ...
  print(f"copy 7 coefficients of type f32")
  ...
  # Uncomment block below to dump debug output
  # Note that debug_util can only be used in simulator runs

  #debug_mod = debug_util(dirname)
  #print(f"=== dump nrm2 with core_fabric_offset_x = {core_fabric_offset_x}, core_fabric_offset_y={core_fabric_offset_y}")
  #for py in range(height):
  #  for px in range(width):
  #    t = debug_mod.get_symbol(core_fabric_offset_x+px, core_fabric_offset_y+py, 'nrm2', np.float32)
  #    print(f"(py, px) = {py, px}, nrm2_ij = {t}")
