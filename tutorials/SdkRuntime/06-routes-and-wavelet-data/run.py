...
# Input and output will be a 16-bit data type
memcpy_dtype = MemcpyDataType.MEMCPY_16BIT
memcpy_order = MemcpyOrder.ROW_MAJOR

# Set up an input tensor to be sent along memcpy H2D streaming color
input_color = 8
input_tensor = np.array([42], dtype=np.int16)

# ISL map to indicate the PE that will receive the input wavelet
input_port_map = "{input[idx=0:0] -> [PE[0,0] -> index[idx]]}"

# Prepare input tensor and perform streaming H2D memcpy
(px, py, w, h, l, data) \
  = runtime_utils.convert_input_tensor(input_port_map, input_tensor)

runner.memcpy_h2d(input_color, data, px, py, w, h, l,
