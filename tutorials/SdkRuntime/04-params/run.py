...
# ISL map to indicate the PE that will produce the output wavelet
...
(px, py, w, h, l, data) \
  = runtime_utils.prepare_output_tensor(output_port_map, np.float16)

runner.memcpy_d2h(data, output_color, px, py, w, h, l,
                  streaming=True, data_type=memcpy_dtype,
                  order=memcpy_order, nonblock=False)

result = runtime_utils.format_output_tensor(output_port_map, np.float16, data)

# Stop the program
runner.stop()

# Ensure that the result matches our expectation
np.testing.assert_allclose(result, [42.0], atol=0.01, rtol=0)
print("SUCCESS!")
